...
from ..models.core import CommandPayload, ErrorResponse, CommandResponse, ParticipateResponse, UnitsRepsonse, WorldResponse, RoundsResponse
import asyncio
...
class GameApi:
    ...
    async def request[T: BaseModel](
        ...
        async with self._session.request(
            ...
            if response.status != 200:
                ...
                context_logger.error(f"Error response: {resp}")
                return resp
            return output_model.model_validate(data)

    async def command(self, payload: CommandPayload) -> CommandResponse | ErrorResponse:
        return await self.request("POST", "/play/zombidef/command", CommandResponse, payload)

    async def participate(self) -> ParticipateResponse | ErrorResponse:
        return await self.request("PUT", "/play/zombidef/participate", ParticipateResponse)
    ...
    async def __aenter__(self) -> "GameApi":
        return self
